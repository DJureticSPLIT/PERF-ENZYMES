...
C  Experimental values are used for beta-lactamase Lac-1 at 0.12s, as in
...
C
C The output of the program Lac1-SimII.for are Lac1-SIexper.dat and Lac1-SimII.dat.
C The first row with numbers from the Lac1-SimII.dat must give the same parameters
C already contained in the single row with numbers from the Lac1-SIexper.dat.
...
C
C
        open(unit=15,status='unknown',file='Lac1-SIexper.dat',
...
C
C
        open (unit=7,status='unknown',file='Lac1-SimII.dat',
     & access='sequential',form='formatted')
       write(7,40)
...
Cn
     ...
     &   (rck3*sigmab))
C
C        enprodT=enprodB1+enprodB2+enprodB3
         enprodT=currB2*2.3025851*dlog10((ckonst1*ckonst2*ck3)/
     &   (rckonst1*rckonst2*rck3))
C entropy production divided by R is in units of inverse seconds
C
         Ptot=enprodB1+enprodB2+enprodB3
C
       write(7,'(i5,1x,6(f12.1),(f10.6,1x),5(f18.6),12(f18.6,1x))')
