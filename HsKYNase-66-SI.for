...
C The substrate is L-kynurenine at the concentration of 0.0001 M. First product is the anthranilic acid.
C The second product is L-alanine.
C The choice of product concentrations is 0.0001 M.
C
C The output of the program HsKYN66-SI.for are three data files:
C a) KYN66SI-exper.dat containing the row #43 parameters from the SI excel dataset (s=sum of probabilities).
C b) KYN66SimI-K1K2K3K4c.dat containing the 2000 rows with the Simulation I results. The first row
C repeats KYN66SI parameters. Rows with negative random numbers (rand) are replaced
C with the KYN66SI-exper.dat row.
C c) KYN66SimII-K1K4k1k7.dat containing the 2000 rows with the Simulation II results. The first row
C repeats KYN66SI parameters. Rows with negative random numbers (300 rows) are replaced with the first
...
85     continue
C
            open (unit=11,status='unknown',file='KYN66SI-exper.dat',
...
C
      close (unit=11)
C
        open (unit=13,status='unknown',file='KYN66SimII-K1K4k1k7.dat',
     & access='sequential',form='formatted')
       write(13,65)
...
C
         ...
         k12s=5900000.0
            ...
            ckonst1a(i)=2.20149*gaussrand1(i)
C
          keqk12=ckonst1a(i)
          k12=268.0*keqk12
          k12s=k12/substrates
...
C
C
        open (unit=9,status='unknown',file='KYN66SimI-K1K2K3K4c.dat',
     & access='sequential',form='formatted')
       write(9,95)
...
C
C The run time should be around 4.0 sec.
